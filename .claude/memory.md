...
| F-17 | 改修: 画面を離れて戻っても直近の run の進捗/結果を表示（#14a を latest に拡張）・ポーリング再試行 | agent | F-6 | DONE | RV-059: **DONE 可**（P1/P2 0・P3 4 修正・残は TODO-061） | 2026-09-25 |
...
- [RV-059] F-17（reviewer サブエージェント 1 回）: **DONE 可**。「画面を移動すると生成が中止になる」の原因は UI 側（バックエンドのジョブは継続・画面が実行中 run しか復帰しなかった）。
  P3 のうち、再試行を一時的な失敗（5xx・通信断）に限定・直近 run の復帰を資料投入画面だけ（`resumeLatest`）・古いコメント削除・failed/stopped の復帰テストを修正。残 P3 は TODO-061。
...
- [TODO-061] RV-059 の記録のみ P3: #14a のエンドポイントがサービスを 2 回呼び `recover_expired` が 2 回走る（`RunService.resume` に集約）／復帰した run では引き継ぎ承諾の注記が消える（#13 に承諾の有無が無い・F-17 以前から）／直近 run が終わっていると資料投入画面を開くたびに案件一覧を 1 回無効化する（許容）。
