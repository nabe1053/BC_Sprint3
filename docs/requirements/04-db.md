...
| `document_exclusions` | 資料の除外（誤投入・差し替え前の資料を以後の受付一覧・案の作成・件数上限から外す）。**資料・抽出結果・根拠は消さない**。1資料1回・取消なし（2026-09-24 追加・F-16・memory AD-036 ①） | FUNC-01 |
...
| field | text | NOT NULL, CHECK IN (編集可能項目のみ) | **原表記・出典・原項番・選択グループは含めない**（③ SCR-04・X11）。**語彙（2026-09-13 確定・memory AD-018）**: `kind`,`usage_note`,`od_value`,`od_unit`,`wall_value`,`wall_unit`,`weight_value`,`weight_unit`,`grade`,`connection`,`range_class`,`length_value`,`length_unit`,`qty_value`,`qty_unit`,`note`。状態は `new_state` で運ぶ。`due_raw`/`place_raw` は原則4（`*_raw` 不変）を優先し初版では編集不可（TODO-018・研修者確認） |
...
| undone_by | text | CHECK(undone_at IS NULL OR trim(undone_by)<>'') | 取消者（2026-09-13 追加・memory AD-017。人の操作は記録者名を必ず持つ・原則5。AI は補完しない） |
...
| id | bigserial | PK | |
| version_id | bigint | NOT NULL, FK→versions | |
| from_state | text | NOT NULL, CHECK IN ('draft','staff_checked','review_checked'), CHECK(from_state <> to_state) | 遷移前。同一状態への「遷移」は行にしない（Build AD-028 ⑪） |
...
| bounce_id | bigint | FK→bounces | 差し戻しとして確定した時点で紐づく。未確定なら NULL。**NULL→値の 1 回限りの UPDATE を追記型の明示的例外として許容**（`undone_at` と同型・差し戻し INSERT と同一トランザクション。値が入った行は二度と変えない。Build AD-028 ⑦） |
...
#### `document_exclusions`（2026-09-24 追加・F-16）

| 列 | 型 | 制約 | 説明 |
|----|----|------|------|
| `id` | bigint | PK | |
| `document_id` | bigint | FK→`documents.id` NOT NULL **UNIQUE** | 1資料1回。取消は初版では作らない |
| `recorded_by` | text | NOT NULL・`CHECK(trim(recorded_by) <> '')` | 除外者（画面の入力値。AI は補完しない） |
| `recorded_at` | timestamptz | NOT NULL | |
| `created_at` | timestamptz | NOT NULL DEFAULT now() | 共通列（D層のため `updated_at` は置かない） |

- 除外は案件の行ロック（`cases FOR UPDATE`）の中で、**実行中の run が無いこと**を確かめてから追記する（#12 起動と同じロック。起動と除外が競合して除外済み資料を読む run を作らない）。実行中なら `E_RUN_IN_PROGRESS`（409）
...
| 論理削除 | 案件・版の削除は初版では想定しない。**記録の削除経路を作らない**（監査可能性の維持）。**資料は 2026-09-24 改訂（F-16・memory AD-036 ①）で「除外」を記録できる**: 物理削除・UPDATE はせず、D層 `document_exclusions` に除外者・日時を追記する。除外済みの資料は受付一覧（#4）・案の作成の入力（#12 の起動検査・`inputDocuments`）・エージェントの読取ツール・件数上限から外し、既存版の根拠・棚卸し・原ファイル取得（#10）からは引き続き参照できる |
...
| 変更・確認記録 | `item_edits` / `confirmations` / `question_judgements`（全履歴） / `version_state_events` / `bounces`＋`bounce_comments` / `sendoff_decisions`（1 記録 = 1 行、取消は列で表す。Build AD-030 ⑮） |

出力ビルダは保存値を**写すだけ**で四則演算・単位変換・丸めを行わない（D03。出力される数値の集合 ⊆ 保存された数値の集合をテストで固定。Build AD-030 ③）。数値は `numeric` を `Decimal` のまま書き、文字列セルは先頭 `=` でも数式化しない。日時は `YYYY-MM-DD HH:MM:SS+09:00` の文字列（Build AD-030 ⑪）。
...
- JSONLは確定済みtrace_eventをseq順に再構築し、同一runのDBロック中に一時ファイルから置換する。DB保存失敗時には出力しない。開始・終了時の出力の再試行で同じ内容へ回復でき、seqを重複追記しない。起動時の再構築は、保存された外側期限と終端保存猶予16秒を過ぎたrunningの回収対象runのみ（AD-023 / LN-056）。
...
- stage_detailの終端・障害の固定診断コード一覧はworker_failed/process_interrupted/job_start_failed/draft_not_finalized/agent_implementation_pending/terminal_recovery/trace_write_failed/tool_rejected/local_dummy_unsupported/validation_unresolved/model_error。tool_rejectedは同一ツール×同一エラーコードが連続3回の拒否等、local_dummy_unsupportedはダミーの解釈範囲外、validation_unresolvedは方針側で未解消の検証違反を示す。model_errorは実モデルSDKの認証・レート制限・通信等の失敗を示す。stop_reasonの9語彙は変更しない。worker_failedはworkerの例外・キャンセル、process_interruptedはジョブ境界自身の中断、またはプロセス起動時の期限切れrun回収を示す。起動時回収は外側期限を過ぎたrunning runのみ（started_at + 保存済みlimits.outerTimeoutS + 終端保存猶予16秒 < now）とし、猶予込み期限内と期限ちょうどのrunは触らない（AD-023 / LN-056）。資料本文・例外内容は格納しない。
- 無応答／内側期限で停止した `job_interrupted` の observation は、既存の status/count/code に `sinceLastHeartbeatS`（直近の生存信号からの秒数。未受信時は実行開始から）と `heartbeats`（実行全体の受信数）の数値だけを追加する。停止判定時に採取し、本文は含めない。
- （2026-09-24 追記・F-1）ツールの引数検証で失敗した step の observation は、既存の status/count/code に `errors`（`[{path, type}]`・最大 20 件）を追加する。`path` は引数内の位置（例 `rows.0.lengthState`）、`type` は検証の固定型（`E_STATE_VALUE_CONFLICT` 等）のみで、検証メッセージ・入力値は格納しない。`job_finish` の result イベントは `stopDetail`（終端時の `stage_detail` の固定診断コード。例 `tool_rejected`。無ければ null）を持つ。
- T-203の読取進捗はstage_detailのJSON文字列 `{"documentsRead": n, "documentsTotal": N}`。総数は案件の資料一覧、読取済みは空でない読取可能範囲がすべて成功走査された資料数。再読取/分割stepを重複計上しない。読取時の固定診断はtrace_event.observation.codeへ保全し、件数JSONと区別する。終端stage=doneは既存の固定診断コード契約を維持する。
