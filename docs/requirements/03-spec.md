...
### 2026-09-24 研修者の画面確認による改訂（memory AD-036・F-12〜F-17）

本節は同日以降の実装の正。各画面の節で旧記述と食い違う場合は本節を優先する。

- **名称と共通表示（F-12）**: 画面左上の名称は「引合書整理エージェント」（マークは「引」）。「SPRINT 3」の副題、画面 ID の眉ラベル（`SCR-xx / …`）、
  設計書名を含むフッター、模擬期の注記（「実ファイルを抽出する処理ではありません…」）、版の履歴の「Scope 2」枠（「2版を比較」の無効ボタン）は**表示しない**。
  「初版は版を別に保持して人が比較します」の注記は残す。処理記録と実行結果には内部 ID（版 ID・資料 ID）・内部のツール名・ms 表記を出さず、処理名と秒で示す。
- **日時（F-12）**: 記録日時（記録・取消・確定・出力・判断）は**日本時間（Asia/Tokyo）の分まで**、`YYYY-MM-DD HH:mm` で表示する。
  見積期限は原表記を示す方針のため対象外（TZ を補わない）。
- **SCR-03 の操作性（F-13）**: 「担当者確認済みにする」（primary）は見出し右の**右端**に置き、未解決件数の注記と失敗表示は見出しの下の行に出す
  （押下の前後でボタンの位置を動かさない）。照合チェック列の見出しは「**出典と照合**」とし、☑の下に照合の記録者と日時（JST）を示し、横スクロールしても左端に残す。
  「確認事項の要約」列は各要約の先頭に確認事項番号を付け（例「Q1 材質を照会」）、要約・判断欄は列内で折り返す。判断欄では要約を繰り返さず番号で対応づける。
  SCR-06 の確認事項欄も列内で折り返す。
- **SCR-01 案件一覧（F-15）**: 「確認事項」列に最新版の「残 n / 全 N」を示す。「表示状態」には最新の状態の記録者・日時（JST）を併記する（当初仕様 SCR-01 の「確認者・確認日時を併記」を実装）。
  各行の「記録を表示」で行を展開し、その版の記録（訂正・照合・網羅性確認・判断・状態・差し戻し・送付可否）を新しい順に記録者・日時つきで一覧する。版が無い案件は「—」で展開しない。
- **左ナビと対象案件セレクタ（F-14）**: 左ナビの 03〜05 は URL の版を優先し、版が URL に無い画面（資料投入）では**その案件の最新の確定版**で開く。
  案件一覧ではこのタブで直前に開いた案件を使う。版がまだ無い案件だけ 03〜05 を無効にし「案を作成すると開けます」と示す（案件も無ければ「案件を選ぶと開けます」）。
  SCR-03/05/06 は見出しの説明文の下に**対象案件セレクタ**を置く（SCR-03 ⑤ AD-024・SCR-05 AD-027・SCR-06 AD-031 の「セレクタは作らず URL で選ぶ」を覆す）。
  選んだ案件の最新の確定版の同じ画面へ移り、版の無い案件は「（案がまだありません）」として選べない。
- **SCR-02 資料投入（F-16）**: 投入・一覧取得・案の作成の失敗は**危険色の枠つき警告**（`shared/ui/StatusNotice`。枠と文字だけに `color-danger` を使い面は塗らない。MUI `Alert` は oklch の状態色を演算できず使わない）で示し、原因と直し方の文言を残す（引き継ぎの確認は注意色）。
  受付一覧の表は枠で囲み、列の区切り（`color-hair`）と最終行の下線を持たせる。各行に「除外」（text ボタン）を置き、確認ダイアログで**除外者名（必須・AI は補完しない）**を入れて除外する。
  除外は物理削除ではなく記録で、受付一覧・次の案の作成・エージェントの読取から外れ、作成済みの版の根拠には残る。取り消せない。案の作成中は除外できない。
  「除外済みの資料を表示」で除外済みの資料と除外者・日時（JST）を一覧する。
- **SCR-02 案の作成の再表示（F-17）**: 案の作成は画面を離れても止まらない（バックエンドのジョブで続く）。資料投入画面を開き直したら、実行中の run の進捗へ戻り、
  実行中が無ければ**直近の run の結果**（「案を作成しました」と新しい版へのリンク、または停止理由）を表示する。次の「案を作成」は押せる。
  進捗のポーリングは一時的な失敗（500・通信断）を 3 回まで再試行し、1 回の失敗で「通信中断」にしない（run が無い 404 は即時に表示）。
...
> 納期・納地・見積期限は「—」＋「現在の案件情報では未提供」の注記（値を補完しない）。③受付エリアは実際に読取し、受付一覧に
> 読取結果 5 区分をラベル文字で表示する（#4/#5）。受付一覧には形式ごとのページ／シート数（PDF・xlsx のみ。判定できなければ「—」）と、一部読取不能の対象範囲（「一部読取不能（p.2）」）も表示する（2026-09-24 追記・TEST-01 #2・TEST-02 #1）。④上限の注記は AD-003 の仮値（資料50件・20MB・PDF 200ページ・xlsx 50シート）。
...
**目的**: 担当者の主作業画面。案件ヘッダと明細一覧を確認し、件数（明細・確認事項のある行・数量TBA・択一・矛盾グループ・照合済み・訂正・未解決）を把握する。**行ごとの一致確認**と**行ごとの確認事項の判断**を同じテーブル上で記録し、値の訂正は根拠ドロワー（SCR-04）で行う。.xlsx の出力と版の履歴もこの画面の見出し右で操作する。担当者の作業が完了したら引合書承認（SCR-06）へ引き渡す。
...
| **版の履歴（開閉パネル）** | 見出し右の開閉式。版／作成日時／状態／未解決／出力の一覧、再実行ボタン、引き継ぎ警告、出力シートの説明を内包する（Scope 2 枠は 2026-09-24 改訂で表示しない） | 見出し右 | FUNC-09, FUNC-10, N04 |
...
| 件数表示 | 明細／確認事項のある行／数量TBA／択一・矛盾グループ（`ALT-n` と `CFL-n` の合計）／**照合済み n/N**／**訂正 n**／**未解決の確認事項 n** | メタ下 | FUNC-06〜08 |
| 担当者名入力 | 一致確認・訂正・判断の記録者名（入力必須）。網羅性確認の記録状況を併記 | テーブル上 | FUNC-08 |
| 絞り込み欄 | キーワード／品種／材質／接続／状態（全件・確認事項あり・数量TBA・択一・矛盾の候補・未照合のみ・訂正済みのみ・**未解決のみ**）／条件をクリア／件数表示 | テーブル上 | FUNC-05, FUNC-07, FUNC-08 |
| Item List テーブル | **19列**。出典と照合（2026-09-24 改訂で「照合」から改称）／行ID／品種／外径／単重／材質／接続／長さ／数量／単位／納期／選択グループ／状態／確認事項の要約／根拠の要約／**対応状況／解決状態／判断内容**／詳細 | 中央（横スクロール） | FUNC-03〜08 |
| 照合チェック | 行ごとの「重要項目を出典と照合した」。チェック時に担当者名を使用。☑の下に記録者・日時（JST）を示し、横スクロールしても左端に固定（2026-09-24 改訂） | テーブル1列目 | FUNC-08 |
...
| 比較の注記 | 「初版は版を別に保持して人が比較します」（「2版を比較」の無効ボタンと Scope 2 の印は 2026-09-24 改訂で削除） | N04 |
...
| 択一候補 | 選択グループあり（`ALT-n` 等） | 選択グループ列にグループID、状態列に「択一」。合計数量は表示しない | FUNC-06 |
| 矛盾候補 | 選択グループが `CFL-n`（資料の記載が相反し優先関係を判定できない併記。04-db group_code・X04） | 選択グループ列にグループID、状態列に「矛盾候補（要判断）」。客先が選ぶ択一と区別する。合計数量は表示しない | FUNC-06 |
...
| **変更・判断事項の一覧**（ドロワー内） | 行ごとに 変更採用（P.S.採用／Rev.訂正）・択一（グループID）・矛盾候補（`CFL-n`）・数量TBA・継承候補・担当者訂正 n件・担当者判断（対応状況／解決状態）をピルで示し、確認事項の理由と判断内容を1行で併記。見出しに行数 | 右ドロワー | FUNC-06〜08 |
...
| 変更・判断事項が無い | 変更・択一・矛盾候補・TBA・継承候補・訂正・判断のある行が0 | 起動ボタンは「0行」を表示したまま残し、開くと「該当する行はありません」。**ボタンを隠さない**（存在しないのか未確認なのかを区別する）。表は全行のまま（照合の必要は残る） | FUNC-08 |
...
| warn / warn-100 / warn-800 | `oklch(0.62 0.11 70)` / `oklch(0.96 0.035 80)` / `oklch(0.40 0.08 65)` | 「択一」「矛盾候補」「TBA」「継承候補」「要確認」「数量未確定」「未解決」「保留」「差し戻し」「訂正」「未照合」等 |
