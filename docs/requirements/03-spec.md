...
> **初版実装の暫定（2026-09-12・研修者決定、memory AD-013）**: G1（T-103）時点では SCR-03 と版が未実装のため、
> ①表示状態・送付可否の列は**常に「—」**を表示し、注記に「版の状態は G3 以降に表示」を添える。
> ②操作列は「投入画面へ」（SCR-02）のみを出し、「案件を開く」（SCR-03 へ）は G3（T-303）で追加する。
> ラベルと遷移先の不一致を作らないための暫定であり、G3 到達時にこの注記を削除して本表に戻す。
...
| 差し戻しで状態が戻ると誤解 | 差し戻し記録後 | 評価状態は担当者確認済みのまま。SCR-03 上部に「差し戻し中（作成案には戻りません）」と行コメントを表示 | FUNC-10 |
| 評価確認済みの後に訂正 | 担当者が訂正を記録 | 状態を担当者確認済みへ戻し、評価確認の日時を消す（再確認が必要であることを示す） | FUNC-08, FUNC-10 |
